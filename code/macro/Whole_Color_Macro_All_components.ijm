...
//Adapted from script written by Chris Strock, 2021-09-30

//user selects working directory
input=getDirectory("Choose Source Directory ");
list = getFileList(input);

// Global accumulator for the pivoted CSV (one row per image_ID).
// Each Wholecolor() call appends one row of mean channel values to this string,
// which is written out to "Whole_Color_Measurements_pivoted.csv" after each image.
var pivotData;
pivotData = "image_ID,mean red,green,blue,hue,saturation,brightness,lightness,a,b\n";
...
//capture the mean for each of the 3 channels just appended (rows are in slice order: R, G, B)
nRGB = nResults;
meanRed   = getResult("Mean", nRGB-3);
meanGreen = getResult("Mean", nRGB-2);
meanBlue  = getResult("Mean", nRGB-1);
...
//capture the mean for each of the 3 channels just appended (rows are in slice order: L, a*, b*)
nLAB = nResults;
meanL     = getResult("Mean", nLAB-3);
meanAstar = getResult("Mean", nLAB-2);
meanBstar = getResult("Mean", nLAB-1);
...
//capture the mean for each of the 3 channels just appended (rows are in slice order: Hue, Saturation, Brightness)
nHSB = nResults;
meanHue        = getResult("Mean", nHSB-3);
meanSaturation = getResult("Mean", nHSB-2);
meanBrightness = getResult("Mean", nHSB-1);
...
//Also build a pivoted CSV with one row per unique image_ID and the mean of each
//channel as its own column. Derive the image_ID from the filename by stripping the
//file extension (e.g. "IMG_2076.CR3" -> "IMG_2076").
   imageID = filename;
   dotIndex = lastIndexOf(imageID, ".");
   if (dotIndex!=-1) imageID = substring(imageID, 0, dotIndex);

//Append this image's row. Values are formatted to 3 decimal places to match the
//"decimal=3" measurement setting. Column order: image_ID, mean red, green, blue,
//hue, saturation, brightness, lightness, a, b.
   pivotData = pivotData + imageID + ","
            + d2s(meanRed, 3) + "," + d2s(meanGreen, 3) + "," + d2s(meanBlue, 3) + ","
            + d2s(meanHue, 3) + "," + d2s(meanSaturation, 3) + "," + d2s(meanBrightness, 3) + ","
            + d2s(meanL, 3) + "," + d2s(meanAstar, 3) + "," + d2s(meanBstar, 3) + "\n";

//Save (overwrite) the pivoted CSV after each image, so partial results survive an
//interrupted run. Saved to the same folder as the images.
   File.saveString(pivotData, dir + "Whole_Color_Measurements_pivoted.csv");
